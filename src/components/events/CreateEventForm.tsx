  ...
  const onSubmit = async (data: EventFormValues) => {
    try {
      ...
      const body = {
        ...
        tags: data.tags ?? [],
        speakers: data.speakers?.map((s: { name: string; bio?: string; role?: string; display_order: number }) => ({
          name: s.name,
          bio: s.bio ?? null,
          role: s.role ?? null,
          display_order: s.display_order,
        })) ?? [],
        agenda: data.agenda?.map((a: { title: string; description?: string; start_time?: string; end_time?: string; location?: string; display_order: number }) => ({
          title: a.title,
