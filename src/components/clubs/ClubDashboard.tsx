...
=======
        ...
        const formattedEvents = eventsData.slice(0, 5).map((event: Record<string, unknown>) => ({
          id: (event.event_id ?? event.eventId) as number,
          title: (event.name ?? "") as string,
          date: new Date((event.datetime as string) ?? "").toLocaleDateString(),
          registrations: (event.current_attendees ?? event.currentAttendees ?? 0) as number,
          capacity: (event.max_attendees ?? event.maxAttendees ?? 0) as number,
          status: (event.status ?? "Open") as string
        }));
        ...
        >
          <div className="space-y-1">
            ...
            <Button 
              variant={activeNavItem === "members" ? "default" : "ghost"} 
              className="w-full justify-start text-sm lg:text-base"
              onClick={() => setActiveNavItem("members")}
            >
              <Users className="mr-2 h-5 w-5" />
              <span>Members</span>
            </Button>
