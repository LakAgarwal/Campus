...
=======
      ...
      const res = await apiFetch<{ club_id: number; club_code?: string }>("/clubs", {
        method: "POST",
        body: JSON.stringify({
          name: values.club_name,
      ...
      if (res.error || !res.data) {
        throw new Error(res.error || "Failed to create club");
      }
      const clubData = res.data;

      setClubCode(generatedCode);
      
      const data = res.data as { club_code?: string };
      setClubCode(data?.club_code ?? null);
      toast({
        title: "Club Created Successfully",
        description: data?.club_code
          ? `Your club '${values.club_name}' has been created! Your club code is ${data.club_code}.`
