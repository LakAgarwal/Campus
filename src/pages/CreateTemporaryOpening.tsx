...
const CreateTemporaryOpening: React.FC = () => {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      const opening = await api.post<{ opening_id?: number; openingId?: number }>('/openings', {
        created_by: user?.id ?? '',
        title: data.title,
      ...
      const openingId = opening?.opening_id ?? (opening as { openingId?: number })?.openingId;
      if (openingId && data.custom_fields?.length > 0) {
        await api.post('/openings/' + openingId + '/optional-details', data.custom_fields.map(field => ({
          heading: field.heading,
          subheading: field.subheading,
          content: field.content
        }))).catch(() => null);
      }
      if (openingId && (data.duration || data.start_time || data.end_time || data.max_people)) {
        await api.post('/openings/' + openingId + '/records', {
      ...
      if (openingId && data.media?.length) {
        await api.post('/openings/' + openingId + '/media', data.media.map(m => ({ media_type: m.type, media_url: m.url }))).catch(() => null);
      }
      if (openingId && data.links?.length) {
        await api.post('/openings/' + openingId + '/links', data.links.map(l => ({ link_type: l.type, url: l.url }))).catch(() => null);
      }
