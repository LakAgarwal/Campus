...
import { EventWithRegistration_1 } from '@/lib/event-types';
...
const EventsPage: React.FC = () => {
  ...
  const { data, isLoading, error } = useQuery<EventsData>({
    ...
    queryFn: async () => {
      ...
      const registrations = await api.get<{ event_id: number; registration_id?: number; status: string; submitted_at?: string; qr_code?: string }[]>('/events/my-registrations') || [];
      const eventIds = registrations.map(reg => reg.event_id);
      if (eventIds.length === 0) {
        return { acceptedEvents: [], pendingEvents: [] };
      }
      const eventsData = await api.get<{ event_id: number; name: string; datetime: string; location: string; club_id: number; short_description: string; payment_link?: string; created_at?: string; event_tags?: { tag: string }[] }[]>('/events') || [];
      const filteredEvents = eventsData.filter((e: { event_id: number }) => eventIds.includes(e.event_id));

      const transformedEvents: EventWithRegistration_1[] = filteredEvents.map((event: { event_id: number; name: string; datetime: string; location: string; club_id: number; short_description: string; payment_link?: string; created_at?: string; event_tags?: { tag: string }[] }) => {
        ...
        return {
          ...
          registration_id: registration?.registration_id != null ? String(registration.registration_id) : '',
          registration_status: (registration?.status as 'accepted' | 'pending' | 'rejected') ?? 'pending',
