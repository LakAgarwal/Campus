...
const ResourcesPage = () => {
  ...
  const fetchResources = async () => {
    try {
      const resourcesData = await api.get<Resource[]>('/resources') || [];
      const userPostedResources = resourcesData.filter((r: Resource) => r.posted_by_type === 'user');
      const userIds = [...new Set(userPostedResources.map((r: Resource) => r.posted_by_user).filter(Boolean))] as string[];
      let profilesData: { id: string; full_name: string }[] = [];
      if (userIds.length > 0) {
        const allProfiles = await api.get<{ id: string; full_name: string }[]>('/profiles');
        profilesData = (allProfiles || []).filter((p: { id: string }) => userIds.includes(p.id));
      }
      const resourcesWithUserInfo = resourcesData.map((resource: Resource) => {
        const processedResource = {
          ...resource,
          processedTags: resource.tags ? resource.tags.split(',').map((tag: string) => tag.trim()) : []
        };
        if (resource.posted_by_type === 'user' && resource.posted_by_user) {
          const userProfile = profilesData.find((p: { id: string }) => p.id === resource.posted_by_user);
          return { ...processedResource, posted_by_name: userProfile?.full_name };
        }
