...
<<<<<<< HEAD
        ...
        const eventData = await api.get<{
          event_id: number; club_id: number; name: string; datetime: string; location: string;
          short_description: string; eligibility: string; registration_deadline: string; status: string;
          max_attendees: number | null; current_attendees?: number; event_thumbnail?: string; event_type?: string; payment_link?: string;
          clubs?: { name: string }; event_tags?: { tag: string }[];
        }>("/events/" + eventIdNumber);
...
=======
        ...
        if (res.error || !res.data) {
          toast({
            title: "Event not found",
            description: res.error || "The event you're looking for doesn't exist or has been removed.",
>>>>>>> 0ac01baa4c622dfc7d74ff1260d588d67ffd0325
            variant: "destructive",
          });
          return;
...
<<<<<<< HEAD
        ...
        const formattedEvent: EventDetails = {
          ...
          status: eventData.status as EventDetails["status"],
...
=======
        ...
        setEvent({
          event_id: (d.event_id ?? d.eventId) as number,
          club_id: (d.club_id ?? d.clubId) as number,
          name: (d.name ?? "") as string,
          datetime: (d.datetime ?? "") as string,
          location: (d.location ?? "") as string,
          short_description: (d.short_description ?? d.shortDescription ?? "") as string,
          eligibility: (d.eligibility ?? "") as string,
          registration_deadline: (d.registration_deadline ?? d.registrationDeadline ?? "") as string,
          status: (d.status ?? "Open") as EventDetails["status"],
          max_attendees: (d.max_attendees ?? d.maxAttendees ?? null) as number | null,
          current_attendees: (d.current_attendees ?? d.currentAttendees ?? 0) as number,
          event_thumbnail: (d.event_thumbnail ?? d.eventThumbnail ?? "") as string,
          club_name: (d.club_name ?? d.clubName ?? "Unknown Club") as string,
          tags: Array.isArray(d.tags) ? (d.tags as string[]) : [],
          speakers: (d.speakers as EventDetails["speakers"]) ?? [],
          contacts: (d.contacts as EventDetails["contacts"]) ?? [],
          links: (d.links as EventDetails["links"]) ?? [],
          sponsors: (d.sponsors as EventDetails["sponsors"]) ?? [],
          prizes: (d.prizes as EventDetails["prizes"]) ?? [],
          agenda: (d.agenda as EventDetails["agenda"]) ?? [],
          resources: (d.resources as EventDetails["resources"]) ?? [],
          media: (d.media as EventDetails["media"]) ?? [],
          faqs: (d.faqs as EventDetails["faqs"]) ?? [],
          optionalDetails: (d.optionalDetails as EventDetails["optionalDetails"]) ?? [],
          socialLinks: (d.socialLinks as EventDetails["socialLinks"]) ?? [],
        });
      } catch {
>>>>>>> 0ac01baa4c622dfc7d74ff1260d588d67ffd0325
        toast({
...
<<<<<<< HEAD
    const checkRegistrationStatus = async () => {
      ...
      if (!userId) return;
      try {
        const regs = await api.get<{ status: string }[]>('/events/' + eventId + '/registrations') || [];
        const myReg = regs.find((r: { user_id?: string }) => r.user_id === userId) as { status: string } | undefined;
  ...
  const handleRegisterClick = async () => {
    try {
      const questionsData = await api.get<unknown[]>('/events/' + eventId + '/questions').catch(() => []);
      setRegistrationQuestions(Array.isArray(questionsData) ? questionsData : []);
...
=======
              <Button 
                ...
                disabled={event.status !== "Open" || hasRegistrationClosed}
                onClick={handleRegister}
              >
                {event.status === "Open" && !hasRegistrationClosed 
                  ? "Register Now" 
