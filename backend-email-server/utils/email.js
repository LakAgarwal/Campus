...
// Fetch user email from Spring Boot API (PostgreSQL backend)
async function fetchUserEmail(userId) {
  try {
    const res = await fetch(`${config.apiUrl}/users/${userId}/email`);
    if (!res.ok) throw new Error(`User email not found: ${res.status}`);
